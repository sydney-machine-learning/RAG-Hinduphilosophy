"""
Bhagavad Gita RAG Evaluation Pipeline — Enhanced with llm2_updated RAG
=======================================================================
...
TRANSLATIONS = [
    ("Eknath-Easwaran", "R1_easwaran_gita.txt"),           # R1
    ("Mahatma-Gandhi",  "R2_gandhi_gita.txt"),             # R2
    ("Purohit-Swami",   "R3_purohit_swami_gita_CLEAN.txt"),# R3
    ("Upanishads-12",   "R4_easwaran_upanishads_CLEAN.json"),# R4
    ("Upanishads-108",  "R5_108_upanishads.json"),         # R5
]
...
RESULTS_DIR    = Path("gita_eval_results_enhanced")  # 新目录，与原脚本区分
QA_JSON        = RESULTS_DIR / "qa_results_enhanced.json"
QA_CSV         = RESULTS_DIR / "qa_results_enhanced.csv"
EVAL_CSV       = RESULTS_DIR / "evaluation_bertscore_enhanced.csv"
NGRAM_CSV      = RESULTS_DIR / "ngram_analysis_enhanced.csv"
COMPARISON_CSV = RESULTS_DIR / "comparison_table_enhanced.csv"
INDEX_BASE     = RESULTS_DIR / "faiss_indices_enhanced"
...
def load_translation(name: str, filename: str) -> List[Document]:
    ...
    if path.suffix.lower() == ".json":
        ...
        # Expected format: {section_key: text_string, ...}
...
def print_summary(results: List[dict], eval_df: pd.DataFrame, ngram_df: pd.DataFrame):
    ...
    print("\n" + "=" * 70)
    print("BHAGAVAD GITA RAG EVALUATION (Enhanced RAG) — SUMMARY")
    print("=" * 70)
